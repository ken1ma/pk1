...
trait Base64 {
	 ...
	 */
	def encode(bin: Seq[Byte]): String = bin.sliding(3, 3).map { window => window.length match {
		case 3 => // 3 bytes
			Seq(
	...
	def encode(text: String, charset: Charset = StandardCharsets.UTF_8): String = encode(text.getBytes(charset))
	...
	def decode(text: String): Array[Byte] = text.filter(ch => ch != '\n' && ch != '\r').sliding(4, 4).map { window =>
		if (window.length != 4)
			throw new IllegalArgumentException(s"text.length is not divisible by 4")
